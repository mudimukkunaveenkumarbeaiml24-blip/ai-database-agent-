...
class Settings(BaseSettings):
    ...
    # Removed DATABASE_URL – we hardcode the path for Vercel compatibility
...
# ── Bulletproof writable database path selection ──────────────────────────
def get_writable_db_path():
    candidates = [
        "/tmp/demo.db",                              # Vercel / Linux
        os.path.join(tempfile.gettempdir(), "demo.db")  # Fallback
    ]
    for path in candidates:
        try:
            ...
            with open(test_file, 'w') as f:
                f.write("test")
            os.remove(test_file)
            # If file exists and is read-only, delete it
            if os.path.exists(path):
                try:
                    ...
                    if "readonly" in str(e).lower():
                        os.remove(path)
                        logging.warning(f"Removed read-only file: {path}")
            return path
        except Exception as e:
            logging.warning(f"Candidate {path} not writable: {e}")
            continue
...
def db_status():
    ...
    return {
        ...
        "error": error,
        "file_exists": os.path.exists(DB_PATH) if DB_PATH != ":memory:" else None,
    }
